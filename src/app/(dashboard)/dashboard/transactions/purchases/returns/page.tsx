...
const toRow = (p: ApiPurchase): PurchaseReturn => ({
    ...
    originalPI: p.originalPurchase?.purchaseNo ?? "—",
...
function PurchaseReturnContent() {
    ...
    const handleExport = () => {
        ...
        exportRowsToExcel(
            "Purchase_Returns",
            COLUMNS.filter((c) => c !== "S.No" && c !== "Actions"),
            paginated.map((r) => [r.returnNo, r.originalPI, r.vendorNo, r.vendorName, r.status, r.source, r.user, r.docDate, r.postingDate, r.assessedValue, r.discount, r.amtExclST, r.salesTax, r.amtInclST, r.furtherTax, r.amtInclFT, r.advanceTax, r.advTaxPercent, r.total]),
        );
                ...
                {paginated.map((r, i) => {
                    ...
                    return (
                        <tr key={r.id}
                            ...
                            <td className="px-3 py-2.5 text-right font-mono text-[#1E293B] dark:text-[#f0f0f0] whitespace-nowrap">{fmt(r.amtExclST)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[#4F5967] dark:text-[#9ca3af] whitespace-nowrap">{fmt(r.salesTax)}</td>
                            <td className="px-3 py-2.5 text-right font-mono font-semibold text-[#A27B3A] whitespace-nowrap">{fmt(r.amtInclST)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[#4F5967] dark:text-[#9ca3af] whitespace-nowrap">{fmt(r.furtherTax)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[#1E293B] dark:text-[#f0f0f0] whitespace-nowrap">{fmt(r.amtInclFT)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[#4F5967] dark:text-[#9ca3af] whitespace-nowrap">{fmt(r.advanceTax)}</td>
                            <td className="px-3 py-2.5 text-right font-mono text-[#4F5967] dark:text-[#9ca3af] whitespace-nowrap">{fmtPercent(r.advTaxPercent)}</td>
                            <td className="px-3 py-2.5 text-right font-mono font-semibold text-[#1E293B] dark:text-[#f0f0f0] whitespace-nowrap">{fmt(r.total)}</td>
                            <td className="px-3 py-2.5">
                                <div className="flex items-center gap-1">
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            router.push(`/dashboard/transactions/ledger/vendor-ledger?purchaseUuid=${r.uuid}`);
                                        }}
                                    ...
                                    <button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            router.push(`/print/purchase/${r.uuid}`);
                                        }}
                                        className="flex h-6 w-6 items-center justify-center rounded-[4px] border border-[#E5E7EB] dark:border-[#3a3a3a] bg-white dark:bg-[#1a1a1a] text-[#4F5967] dark:text-[#9ca3af] hover:bg-[#FAF6F0] dark:hover:bg-[#333] hover:text-[#A27B3A] transition-colors cursor-pointer"
                                    >
