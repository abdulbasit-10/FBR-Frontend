            ...
            <div className="relative z-10 w-full max-w-195 rounded-[14px] border border-[#E5E7EB] bg-white shadow-2xl flex flex-col overflow-hidden max-h-[85vh]">
                ...
                <div className="flex items-center gap-3 px-5 py-4 border-b border-[#F3F4F6] shrink-0">
                    ...
                    <div className="flex-1">
                        <p className="text-[14px] font-bold text-[#1E293B]">Select Purchase Invoice For Return</p>
                ...
                <div className="flex items-center gap-2 px-5 pt-4 pb-3 shrink-0">
                    ...
                    <button onClick={loadInvoices} className="flex h-9 items-center gap-1.5 rounded-[6px] border border-[#E3D2BA] bg-white px-3 text-[12px] font-medium text-[#A27B3A] hover:bg-[#FAF6F0] transition-colors">
                ...
                <div className="flex-1 overflow-auto min-h-40">
                    <table className="w-full text-[12px] min-w-155">
                        ...
                        <tbody className="divide-y divide-[#F3F4F6]">
                            {isLoading ? (
                                <tr><td colSpan={8} className="py-12 text-center"><LogoSpinner label="Loading invoices..." className="mx-auto" /></td></tr>
                            ) : paginated.length === 0 ? (
                                <tr><td colSpan={8} className="py-10 text-center text-[12px] text-[#9CA3AF] italic px-6">
                                    No posted purchase invoices with returnable quantities. Post a purchase invoice first, or complete existing returns.
                                </td></tr>
                            ) : (
                                paginated.map((inv, i) => (
                                    <tr key={inv.id} className={cn("cursor-pointer transition-colors", i % 2 === 0 ? "bg-white" : "bg-[#FAF6F0]/30", "hover:bg-[#FAF6F0]")} onClick={() => { onSelect(inv); onClose(); }}>
                                        <td className="px-4 py-2.5">
                                            <button onClick={(e) => { e.stopPropagation(); onSelect(inv); onClose(); }} className="h-4 w-4 rounded-full border-2 border-[#C69A52] bg-white hover:bg-[#C69A52] transition-colors block" aria-label={`Select ${inv.invoiceNo}`} />
                                        </td>
                                        <td className="px-4 py-2.5 font-medium text-[#1E293B]">{inv.invoiceNo}</td>
                                        <td className="px-4 py-2.5 font-semibold text-[#1E293B]">{inv.vendor}</td>
                                        <td className="px-4 py-2.5 text-[#4F5967]">{inv.vendorNo}</td>
