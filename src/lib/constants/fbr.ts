/**
 * FBR Digital Invoicing constants — mirrors FBR-Backend/src/constants/fbr.ts.
 * Sourced from PRAL Technical Specification for DI API — v1.12 (§9).
 */

export const FBR_SANDBOX_SCENARIOS: { id: string; description: string; saleType: string }[] = [
    ...
    { id: "SN025", description: "Drugs sold at fixed ST rate under serial 81 of Eighth Schedule Table 1", saleType: "Non-Adjustable Supplies" },
    { id: "SN026", description: "Sale to End Consumer by retailers (Standard Rate)", saleType: "Goods at Standard Rate (default)" },
    { id: "SN027", description: "Sale to End Consumer by retailers (3rd Schedule)", saleType: "3rd Schedule Goods" },
    { id: "SN028", description: "Sale to End Consumer by retailers (Reduced Rate)", saleType: "Goods at Reduced Rate" },
];

/** Distinct sale-type strings extracted from FBR_SANDBOX_SCENARIOS, useful for pickers. */
export const FBR_SALE_TYPES: string[] = Array.from(
    new Set(FBR_SANDBOX_SCENARIOS.map((s) => s.saleType)),
);

/** Common tax-rate presets. FBR expects the exact ratE_DESC string from /pdi/v2/SaleTypeToRate. */
export const FBR_COMMON_RATES: string[] = ["18%", "17%", "10%", "5%", "3%", "1%", "0%"];
